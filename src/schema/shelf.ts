...
export const addItemForm = z.object({
  ...
  author: z.string().min(1, 'Author is required').max(120, 'Author must be less than 120 characters'),
...
export const addFolderForm = z.object({
  name: z.string().min(1, 'Folder name is required').max(60, 'Folder name must be less than 60 characters'),
  isPublic: z.boolean()
});
